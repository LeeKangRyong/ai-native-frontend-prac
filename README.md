# AI Native Frontend 개발 연습

> 한 명의 FE 개발자가 4개 앱을 Claude Code로 자동화 개발하는 레포지토리

`DRIVER-APP`, `USER-APP`, `MANAGER-WEB`, `INTRO-WEB`

![Claude Code](https://img.shields.io/badge/built%20with-Claude%20Code-D97757)

## 목차

- [레포지토리 설명](#레포지토리-설명)
- [기술 스택](#기술-스택)
- [목표](#목표)
- [Prerequisites](#prerequisites)
- [Quick Start](#quick-start)
- [세팅](#세팅)
- [격리 규칙](#격리-규칙)
- [개발 워크플로우](#개발-워크플로우)
- [커밋 & 브랜치 컨벤션](#커밋--브랜치-컨벤션)
- [.ai/ 문서 구조](#ai-문서-구조)
- [Custom Skills](#custom-skills)
- [참고 문서](#참고-문서)

---

## 레포지토리 설명

하나의 서비스에 대해 4가지의 Frontend application이 있다.

- `prac-fe-app-driver`: 기사님들용 모바일 앱
- `prac-fe-app-user`: 사용자용 모바일 앱
- `prac-fe-web-manager`: 관리자용 웹
- `prac-fe-web-intro`: 서비스 소개 웹

---

## 기술 스택

| 앱 | 프레임워크 | 언어 | Node |
|---|---|---|---|
| `prac-fe-app-driver` | React Native (Expo) | TypeScript | 20 |
| `prac-fe-app-user` | React Native (Expo) | TypeScript | 20 |
| `prac-fe-web-manager` | React + Vite (SPA) | TypeScript | 22 |
| `prac-fe-web-intro` | Next.js (App Router) | TypeScript | 22 |

---

## 목표

한 명의 FE 개발자가 스케일이 큰 FE 작업을 자동화하여 혼자 개발할 수 있도록 한다.  
`claude --dangerously-skip-permissions`를 걸어둬야 자동화할 수 있으므로, 각 서비스 간 간섭이 없도록 해야 한다.  
FE 개발 + 디자인을 24시간 돌리면서, 다른 작업을 수행할 수 있도록 한다.

---

## Prerequisites

- **WSL2** (Windows) — 메인 개발 환경
- **Node.js** — 모바일 앱 20, 웹 앱 22 (`nvm` 권장)
- **tmux** — 4앱 병렬 세션
- **Claude Code** — `claude --dangerously-skip-permissions`로 자동화 실행
- **Expo CLI** — React Native 앱 (`prac-fe-app-driver`, `prac-fe-app-user`)
- **gh CLI** — 이슈 발행 및 커밋·푸시 스킬

> Windows에서 WSL을 쓸 수 없는 경우, Desktop 단일 인스턴스 모드로 폴백한다 (tmux 4분할 대신 앱 하나씩 작업).

---

## Quick Start

```bash
# 1. 레포 클론 후 진입
cd ~/ai-native-frontend-prac

# 2. tmux 4분할 세션 자동 세팅 및 Claude Code 자동 실행 (danger mode)
./start-tmux.sh
```

---

## 세팅

### WSL + tmux

프로젝트 경로: `/home/kangr/ai-native-frontend-prac`

`start-tmux.sh`를 통해 tmux 자동 세팅 및 세션 연결을 수행한다.

```
+------------------------+----------------------+
| 1                      | 3                    |
| prac-fe-app-user       | prac-fe-app-driver   |
+------------------------+----------------------+
| 2                      | 4                    |
| prac-fe-web-manager    | prac-fe-web-intro    |
+------------------------+----------------------+
```

### 브랜치 전략

각 앱은 독립 브랜치에서 개발한다.

| 앱 | 브랜치 |
|---|---|
| `prac-fe-app-driver` | `driver-app` |
| `prac-fe-app-user` | `user-app` |
| `prac-fe-web-manager` | `manager-web` |
| `prac-fe-web-intro` | `intro-web` |
| 루트 설정 변경 | `main` |

### GitHub Actions

각각 push를 하면 다음 step을 통해 CI가 실행된다.

**React Native 앱** (`prac-fe-app-driver`, `prac-fe-app-user`)

```
1. Node.js 20 설치 (package-lock.json 캐시)
2. npm ci
3. npm run lint
4. npx tsc --noEmit
5. npm test -- --watchAll=false
```

**Web** (`prac-fe-web-manager`, `prac-fe-web-intro`)

```
1. Node.js 22 설치 (package-lock.json 캐시)
2. npm ci
3. npm run lint
4. npm test -- --watchAll=false
```

---

## 격리 규칙

> **절대 규칙:** 현재 작업에 할당된 디렉토리 밖의 코드는 **절대 수정·참조하지 않는다.**  
> 각 서비스는 독립적으로 빌드·배포·실패해야 하며, 공유 상태나 cross-app import가 없어야 한다.  
> 이 규칙은 `--dangerously-skip-permissions` 자동화의 안전성을 보장하는 근간이다.

---

## 개발 워크플로우

이슈 발행부터 커밋까지 skill 체인으로 자동화된다.

```
┌─────────────────────┐   ┌─────────────────────┐   ┌──────────────────────────┐   ┌────────────────────┐
│ frontend-issue-     │ → │ figma-design-       │ → │ frontend-implement-      │ → │ frontend-commit-   │
│ publisher           │   │ workflow            │   │ harness                  │   │ push               │
│ (이슈 생성)          │   │ (Figma 화면 설계)    │   │ (6단계 구현 파이프라인)    │   │ (커밋 & 푸시)       │
└─────────────────────┘   └─────────────────────┘   └──────────────────────────┘   └────────────────────┘
                                                      └ 내부에서 frontend-tdd-runner +
                                                        frontend-commit-push 자동 호출
```

각 단계는 단독 실행도 가능하다. `확인만` 또는 `dry-run`을 덧붙이면 코드 변경 없이 파이프라인을 미리 볼 수 있다.

---

## 커밋 & 브랜치 컨벤션

**커밋** (AngularJS 스타일)

```
[type]([scope]): [한글 제목 ≤50자] #[issue]

예) feat(user): 로그인 화면 추가 #12
    fix(driver): 실시간 경로 추적 오류 해결 #3
    chore(root): 루트 커밋 컨벤션 정의
```

**브랜치 네이밍**

```
[type]([scope])/#[issue]-[description]

예) feat(user)/#12-login-screen
```

| 항목 | 규칙 |
|---|---|
| type | `feat` `fix` `docs` `style` `refactor` `perf` `test` `build` `ci` `chore` `revert` |
| scope | `driver` `user` `manager` `intro` `root` |
| issue | GitHub 이슈 번호 (서브앱 필수, root 선택) |

> 커밋·푸시는 `frontend-commit-push` skill이 scope를 자동 감지하고 issue 번호를 검증한다. **PR은 생성하지 않는다.**

---

## .ai/ 문서 구조

4개 앱 모두 `.ai/` 디렉토리를 가지며, AI 워크플로우의 컨텍스트 소스로 사용된다.  
스킬들이 이 파일들을 파싱하여 디자인 제안 및 코드 생성의 기반으로 사용한다.

| 파일 | 역할 |
|---|---|
| `API.md` | 엔드포인트, 데이터 구조 → TypeScript 타입 및 목 데이터 기반 |
| `DESIGN.md` | 디자인 토큰, 화면 목록, 컴포넌트 라이브러리 |
| `CONVENTIONS.md` | 폴더 구조, 네이밍 규칙 |
| `IMPLEMENT.md` | 구현 하네스 진입점 — Entry Conditions, Completion Criteria, Skill Chain 정의 |

---

## Custom Skills

다섯 가지 skill이 있다. 각각 단독으로 사용할 수 있으며, 필요에 따라 `frontend-issue-publisher` → `figma-design-workflow` → `frontend-implement-harness` 순으로 조합할 수 있다.  
(`frontend-implement-harness` 내부에서 `frontend-tdd-runner`와 `frontend-commit-push`를 자동 호출한다.)

> 모든 skill은 `확인만` 또는 `dry-run`을 덧붙이면 코드 변경 없이 실행 계획만 미리 볼 수 있다.

### frontend-issue-publisher

`.github/ISSUE_TEMPLATE/task_ex.md` 템플릿에 맞게 issue를 제작하고 발행하는 skill.  
trigger: `이슈 만들어줘`, `이슈 등록해줘` 등

### figma-design-workflow

`.ai/API.md`, `.ai/DESIGN.md`, `.ai/CONVENTIONS.md`를 파싱하여 디자인 제안을 구조화하고, 인간 승인 후 Figma MCP로 화면을 생성하는 skill.  
모든 화면은 **390×844 (iPhone 14)** 기준으로 생성되며, Safe Area (상단 **59px**, 하단 **34px**) 오버레이를 자동 적용한다.  
trigger: `디자인 시작해줘`, `figma 그려줘`, `화면 디자인해줘` 등

### frontend-implement-harness

`.ai/IMPLEMENT.md`를 읽어 6단계 자동화 파이프라인을 실행하는 skill.  
Planning → Coding → Static Gate → Code Review → Testing → Commit 순으로 전문 에이전트(Planner, Critic, Executor, Reviewer, Debugger, Verifier)를 순차 spawn하며, 완료 조건 100% 충족 후 `frontend-commit-push`로 자동 커밋한다.  
trigger: `구현 시작해줘`, `harness 돌려줘`, `implement`, `개발 시작해줘`

### frontend-tdd-runner

지정된 서브앱의 전체 테스트 게이트를 실행하는 skill.  
두 가지 모드로 동작한다.

- **Validate-only** (기본): tsc → unit test → E2E 순서로 실행, 실패 시 에러 리포트만 출력
- **Auto-fix**: 실패 시 fix-agent가 자동 수정 후 재실행 (unit 최대 3회, E2E 최대 2회)

trigger: `테스트 돌려줘`, `TDD 검증해줘` (validate-only) / `테스트 고쳐줘`, `테스트 통과시켜줘` (auto-fix)

### frontend-commit-push

lint → `frontend-tdd-runner` pre-flight 통과 후 AngularJS 컨벤션에 맞게 커밋하고 앱별 브랜치에 직접 push하는 skill.  
scope 자동 감지, 멀티 서브앱 변경 시 isolation 위반 경고, issue 번호 필수 검증.  
trigger: `커밋하고 푸시해줘`, `작업 끝났으니 올려줘` 등

---

## 참고 문서

- [`DEVELOPMENT.md`](./DEVELOPMENT.md) — 환경 설정, Git/브랜치/이슈 워크플로우 상세
- 각 앱 `CLAUDE.md` / `.ai/` — 앱별 컨벤션·디자인·API 컨텍스트
