# User App Guide (prac-fe-app-user)
...
- Rule: Do not import any files from outside this `prac-fe-app-user` directory.

## Execution Context

This Claude instance runs with `prac-fe-app-user/` as the working directory (tmux 2×2 layout).
- All relative paths use `./` as the base (this directory)
- Monorepo root is one level up: `../`
- Git scope for this pane is always `user` — no scope detection needed
